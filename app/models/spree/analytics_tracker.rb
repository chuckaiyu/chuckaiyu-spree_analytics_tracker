module Spree
  class AnalyticsTracker < Spree::Base
    ...
    def self.current(engine = nil, store = nil)
      ...
      tracker = Rails.cache.fetch("current_analytics_tracker/#{engine}/#{store.id}") do
    ...
    def clear_cache
      TRACKING_ENGINES.each do |engine|
        Rails.cache.delete("current_analytics_tracker/#{engine}/#{store_id}")
      end
    end
  end
end
